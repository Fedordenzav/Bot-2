...
async def command_mem(ctx:commands.Context):
    ...
    await ctx.send("Лови мемчик!", file=images)

@bot.command("gifs")
async def command_gif(ctx:commands.Context):
    gifs = os.listdir("gifs")
    gifs_name = random.choice(gifs)
    with open("gif/"+gifs_name  , "rb") as file:
        image = discord.File(file)
    await ctx.send("Лови гифку", file=gifs)
